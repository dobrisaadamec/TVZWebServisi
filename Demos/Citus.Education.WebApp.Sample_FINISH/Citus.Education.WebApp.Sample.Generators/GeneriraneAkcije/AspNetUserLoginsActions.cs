...
{
	public class AspNetUserLoginsActions
	{
				/// <summary>
		        ...
		        /// <returns></returns>
				public List<AspNetUserLogins> GetAll(int pagecount, int pageindex)
		        {
		            List<AspNetUserLogins> lista = null;
		
		            using (Entities data = new Entities())
		            {
		                var items = data.AspNetUserLogins.Where(r => r.Deleted == false).OrderBy(r => r.Id).Skip(pagecount * (pageindex-1)).Take(pagecount);
		                lista = items.ToList();
		            }
